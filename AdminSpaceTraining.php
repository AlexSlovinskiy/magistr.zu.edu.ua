...
if (!isset($_SESSION["login"]))
	...
	if (isset($_POST['add_tr']))
		...
		if ($error==0)
    		...
    		if (mysql_num_rows($sql)>0)
    			{
    			$success='<h1 class="block" style="background:rgb(212,12,12)"> Галузь знань &bdquo;'.$training.'&ldquo; вже є в базі!</h1>';
    			}
    		else
    			{    			$faculties=htmlspecialchars(serialize($faculty_ids));				$query = "INSERT INTO `trainings`
   				...
   				$sql = mysql_query($query) or die(mysql_error());
   				$success='<h1 class="block"> ГАЛУЗЬ ЗНАНЬ УСПІШНО ЗАНЕСЕНО ДО БАЗИ ДАНИХ!</h1>';
				unset($_POST);
				}
			}
   ...
   if (isset($_POST['del_trn']))
        ...
        if ($_POST["del_trn_ok"]!="+") $error++;
   		if ($error>0)
			{
			$success='<h1 class="block" style="background:rgb(212,12,12)"> ВИБЕРІТЬ ГАЛУЗЬ ЗНАНЬ ТА ПІДТВЕРДІТЬ ВИДАЛЕННЯ!</h1>';
			}
        if ($error==0)
			{			$query = "DELETE FROM `trainings` WHERE `training_name` = '".$training."' LIMIT 1";
			$sql = mysql_query($query) or die(mysql_error());
			$success='<h1 class="block"> ГАЛУЗЬ ЗНАНЬ ВИДАЛЕНО З БАЗИ ДАНИХ!</h1>';
			unset($_POST);
			}
   		}
 ...
 if (isset($_POST['edit_trn']))
		...
		if ($error==0)
			...
			$sql = mysql_query($query) or die(mysql_error());
			$success='<h1 class="block"> ГАЛУЗЬ ЗНАНЬ УСПІШНО ВІДРЕДАГОВАНО!</h1>';
			unset($_POST);
			}
 		}
...
<!-- B.2 MAIN CONTENT -->
        	...
        	<!-- Pagetitle -->
        		<h1 class="pagetitle">Галузі знань</h1>
					<div class="column1-unit">
						...
						echo"</pre>";*/
						echo $success;
						include("Parts/FormTraining.php");
  						?>
