...
<table align=center width="900px" cellspacing=0 cellpadding=0 border=0>
	<tr>
		...
		<tr>
			...
			<td align=center valign=center>��������</td>
			<td align=center valign=center>ͳ������</td>
			<td align=center valign=center>����������</td>
			<td align=center valign=center>���������</td>
			<td align=center valign=center>������</td>
		</tr>
...
<?php
	...
	while ($spc = mysql_fetch_assoc($sql1))
		{
		if (substr($spc['speciality_name'],0,1)=="8")
			{
			echo "<tr>";
			echo "<td align='center'>".$i."</td>";
            echo "<td>".$spc['speciality_name']."</td>";

			$query = "SELECT SQL_CALC_FOUND_ROWS DISTINCT `lastname` , `firstname` , `patronymic` , `pasp_number` FROM `abiturients` WHERE `speciality` = '".$spc['speciality_name']."' AND `language`='���������' ";
    		...
    		$query = "SELECT SQL_CALC_FOUND_ROWS DISTINCT `lastname` , `firstname` , `patronymic` , `pasp_number` FROM `abiturients` WHERE `speciality` = '".$spc['speciality_name']."' AND `language`='ͳ������' ";
			$sql = mysql_query($query) or die(mysql_error());
			$sql_rows= mysql_query("SELECT FOUND_ROWS()") or die(mysql_error());
   			$num_of_rows=mysql_fetch_assoc($sql_rows);
   			$num_of_rows=$num_of_rows["FOUND_ROWS()"];
...
<?php
	...
	while ($spc = mysql_fetch_assoc($sql1))
		...
		if (substr($spc['speciality_name'],0,1)=="8")
			...
			$query = "SELECT SQL_CALC_FOUND_ROWS DISTINCT `lastname` , `firstname` , `patronymic` , `pasp_number` FROM `abiturients` WHERE `speciality` = '".$spc['speciality_name']."' AND `language`='���������' AND `apply`='+' AND `study_type`='�����'";
			$sql = mysql_query($query) or die(mysql_error());
			$sql_rows= mysql_query("SELECT FOUND_ROWS()") or die(mysql_error());
   			$num_of_rows=mysql_fetch_assoc($sql_rows);
   			$num_of_rows=$num_of_rows["FOUND_ROWS()"];
    		echo "<td align='center'>".$num_of_rows."/";
    		$query = "SELECT SQL_CALC_FOUND_ROWS DISTINCT `lastname` , `firstname` , `patronymic` , `pasp_number` FROM `abiturients` WHERE `speciality` = '".$spc['speciality_name']."' AND `language`='���������' AND `apply`='+' AND `study_type`='������'";
			$sql = mysql_query($query) or die(mysql_error());
			$sql_rows= mysql_query("SELECT FOUND_ROWS()") or die(mysql_error());
   			$num_of_rows=mysql_fetch_assoc($sql_rows);
    		...
    		$query = "SELECT SQL_CALC_FOUND_ROWS DISTINCT `lastname` , `firstname` , `patronymic` , `pasp_number` FROM `abiturients` WHERE `speciality` = '".$spc['speciality_name']."' AND `language`='ͳ������' AND `apply`='+' AND `study_type`='�����'";
			$sql = mysql_query($query) or die(mysql_error());
			$sql_rows= mysql_query("SELECT FOUND_ROWS()") or die(mysql_error());
   			$num_of_rows=mysql_fetch_assoc($sql_rows);
   			$num_of_rows=$num_of_rows["FOUND_ROWS()"];
			...
			$query = "SELECT SQL_CALC_FOUND_ROWS DISTINCT `lastname` , `firstname` , `patronymic` , `pasp_number` FROM `abiturients` WHERE `speciality` = '".$spc['speciality_name']."' AND `apply`='+' AND `study_type`='�����'";
			$sql = mysql_query($query) or die(mysql_error());
			$sql_rows= mysql_query("SELECT FOUND_ROWS()") or die(mysql_error());
   			$num_of_rows=mysql_fetch_assoc($sql_rows);
		   	$num_of_rows=$num_of_rows["FOUND_ROWS()"];
