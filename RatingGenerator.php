...
?>
	<b>���
	<?php
		if ($mag==1 && $spc!=-1) echo '&bdquo;������&ldquo;';
		if ($mag==-1 && $spc!=-1) echo '&bdquo;���������&ldquo;';
	?>
...
if ($row["show_results"]=="+")
	{
	echo '<table align=center class=striped cellspacing=0 cellpadding=0 border=1>
		<THEAD>
		<tr class=p>
	...
	$query = "SELECT * FROM `abiturients` WHERE
 		`speciality` = '".$spc."' AND
        `study_type` = '".$type."' AND
		`take_away` = '-' AND `bal1` >= 124 ";
					if ($mag==1) $query=$query."AND `bal2` >= 124 ";

					$query=$query."ORDER BY `benefits`, `bal1`+`bal2`+`bal3`+`dip_average` DESC, `dip_honour`, `dip_average` DESC , `lastname` ASC";
		...
		if (mysql_num_rows($sql)>0)
   			...
   			while ($row = mysql_fetch_assoc($sql))
				...
				echo "<td>".$sum."</td>";
				echo "<td align=center>".$row["dip_honour"]."</td>";
				echo "</tr>";
                $i++;
   				}
   			}
